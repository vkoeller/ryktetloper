...
type GameHandler(logger: ILogger<GameHandler>, applicationLifetime: IHostApplicationLifetime) =
    ...
    let createGame (game: Game) =
        MailboxProcessor<GameActions>.Start(fun inbox ->
            let roundTimer = new Timer(TimeSpan.FromSeconds 1.) //TODO change to 1 minut
            roundTimer.Elapsed.AddHandler(fun _ _ -> inbox.Post  RoundTick)
            roundTimer.AutoReset <- true
            let rec loop (state: Game) =  
                async {
                    match! inbox.Receive() with
                    | AddPlayer (name, connection) when not state.started ->
                    ...
                    | SubmitRound (notebookId, page) ->
                        ...
                        if page.roundNum = state.currentRound then
                            ...
                            if allSubmitted then
                                return! loop { state with roundSecRemaining = 0 }
                        return! loop state
                    | NextRound ->
                        ...
                        roundTimer.Start()
                        return! loop { state with currentRound = nextRoundNum
                                                  started = true 
                    ...
                    | StartGame ->
                        ...
                        if (numRounds % 2) = 0 then //Start by drawing in your own notebook if even number of players
                            ...
                            state.adminConnection.send event
                            roundTimer.Start()
                            return! loop { state with notebooks = initNotebook
                                                      started = true
                                                      roundSecRemaining = ROUND_SEC } 
                    ...
                    | AddAdmin connection ->
                        return! loop { state with adminConnection = connection }
                    | RoundTick ->
                        if 0 < state.roundSecRemaining then
                            ...
                            sendAllPlayers state event
                            state.adminConnection.send event
                            return! loop { state with roundSecRemaining = remaining }
                        else
                            roundTimer.Stop()
                            inbox.Post RoundEnded
                            return! loop { state with roundSecRemaining = -1 }
                    | RoundEnded ->
